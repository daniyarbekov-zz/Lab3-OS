...
open_clientfd(char *hostname, int port)
{
	...
	/* Loop is necessary to grow buffer if it's currently not big enough for
	 * gethostbyname_r */
	buf_len = 1024;
	do {
		buf = Malloc(buf_len);
		rc = gethostbyname_r(hostname, &hent, buf, buf_len, &hp,
				     &h_errno_local);
		if (rc == ERANGE) {
			...
			fprintf(stderr, "%s: line %d: gethostbyname_r: "
				"DNS error %d %d. Check your hostname.\n",
				__FUNCTION__, __LINE__, rc, h_errno_local);
			exit(1);
		}
	} while (0 != rc);
	if (NULL == hp || NULL == hp->h_addr) {
		fprintf(stderr, "%s: line %d: hp is invalid. "
